...
def get_gpu_info():
    try:
        result = subprocess.run(['nvidia-smi', '--query-gpu=count,utilization.gpu,memory.used,memory.total', '--format=csv,nounits,noheader'], capture_output=True, text=True)
        output = result.stdout.strip().split('\n')
        gpu_info = []
        for line in output:
            gpu_count, gpu_util, mem_used, mem_total = line.split(', ')
            gpu_info.append({
